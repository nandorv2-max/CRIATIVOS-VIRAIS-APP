...
const EditModal: React.FC<EditModalProps> = ({ isOpen, onClose, imageUrl, onApplyEdit }) => {
    ...
    useEffect(() => {
        if (!isOpen) {
            ...
            setEditedImageUrl(imageUrl); // Start with the original image
        }
    ...
    return (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-brand-dark rounded-2xl p-6 border border-brand-accent shadow-2xl w-full max-w-7xl relative flex flex-col md:flex-row gap-6 h-[90vh]">
                {/* Image display area */}
                <div className="flex-1 flex items-center justify-center bg-black rounded-lg overflow-hidden relative" onMouseEnter={() => setIsHoveringCanvas(true)} onMouseLeave={() => setIsHoveringCanvas(false)}>
                    ...
                    />
                    {isHoveringCanvas && (
                        <div className="absolute rounded-full border-2 border-white bg-white/30 pointer-events-none" style={{ top: cursorPos.top, left: cursorPos.left, width: brushSize, height: brushSize, transform: `translate(-${brushSize / 2}px, -${brushSize / 2}px)` }} onMouseMove={(e) => setCursorPos({ top: e.clientY, left: e.clientX })}></div>
                    )}
                </div>
