...
const PhotoDisplay: React.FC<PhotoDisplayProps> = ({ era, imageUrl, onDownload, onRegenerate, onEdit, isPolaroid = true, index = 0, showLabel = true, onSaveToGallery, canSaveToGallery }) => {
    ...
    const containerClass = isPolaroid ? `relative group bg-brand-polaroid p-3 pb-12 shadow-xl transform transition-all duration-300 hover:shadow-2xl hover:scale-105 ${rotation}` : 'relative group pb-4 bg-brand-light rounded-xl shadow-lg transition-all duration-300 hover:shadow-2xl hover:scale-105';
    const imageContainerClass = isPolaroid ? 'aspect-square bg-gray-200' : 'rounded-t-xl overflow-hidden';
    const textClass = isPolaroid ? 'text-center mt-4 font-caveat text-3xl text-brand-dark/80 absolute bottom-3 left-0 right-0' : 'text-center mt-3 text-lg font-semibold text-gray-200 px-3';
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className={containerClass}>
            ...
            <div className="absolute top-3 right-3 z-10" ref={menuRef}>
                ...
                {isMenuOpen && (
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.1 }} className="absolute right-0 top-12 mt-2 w-48 origin-top-right bg-brand-dark/80 backdrop-blur-md rounded-lg shadow-2xl ring-1 ring-white/10 text-white text-sm flex flex-col p-1">
                        <span className="w-full text-left px-3 pt-2 pb-1 text-xs text-gray-400 uppercase tracking-wider">Ações</span>
                        <button onClick={() => { onRegenerate(); setIsMenuOpen(false); }} className="w-full text-left px-3 py-2 hover:bg-brand-primary/20 rounded-md transition-colors">Gerar Novamente</button>
                        ...
                        <button onClick={() => { onDownload(imageUrl, era, '9:16'); setIsMenuOpen(false); }} className="w-full text-left px-3 py-2 hover:bg-brand-primary/20 rounded-md transition-colors">Retrato (9:16)</button>
                    </motion.div>
                )}
            </div>
        </motion.div>
